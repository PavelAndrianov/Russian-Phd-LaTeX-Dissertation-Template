...
По определению $\epp_T$ все остальные переходы в окружении должны были содержать в себе абстрактную дугу, которая соответсвует переходу в потоке, то есть $q_i = (create, l_{\nu})$.
Основной вопрос заключается в том, будут ли новые состояния совместными (в смысле оператора $check_{C}$) друг с другом.
...
Таким образом, $\forall 0 \le i \le m, check((s_j, t_j), \{(s_0, t_0), \dots, (s_m, t_m)\}) = true$, а значит, оператор $\epp$ и оператор $apply$ применимы.
Поэтому, 
$$\tau' \in  \epp_{T}
\left(
\begin{pmatrix}
...
При установлении нового соединения функция \textit{isert_connect_request} создавала структуру данных, соответствующую новому соединению, инициализировала ее, потом под мьютексом добавляла в разделяемый список и в конце будила спящий поток \textit{isert_accept_np}, который начинал работать с новым соединением.
Однако, в случае нескольких созданных запросов после создания одного соединения проснутся сразу несколько потоков \textit{isert_accept_np}.
