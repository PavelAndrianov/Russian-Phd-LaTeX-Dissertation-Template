...
Смысл этого требования в том, чтобы выполнялся следующий инвариант: либо множество $\overline{R}$ находится во множестве $W$, либо переходы из этого множества уже рассмотрены и соответствующие состояния находятся во множестве достижимых состояний $R$.

%\iffalse %The following text is banned by moderator
%Требование~\ref{cpa_update_transfer_eq} является очень общим, хотя можно использовать более частное и понятное условие на оператор $update$.
%
%\begin{equation}
...
%\end{equation}
%
%Покажем, что условие ~\ref{cpa_update_transfer_eq_power} влечет за собой выполнение условия~\ref{cpa_update_transfer_eq}.
%Обозначим $\overline{W} = \{\widehat{R} \mid \widehat{R} \in W \land (e, \pi) \notin \widehat{R}\}$.
%$W = \overline{W} \cup \{\widehat{R} \mid \widehat{R} \in W \land (e, \pi) \in \widehat{R}\}$.
...
%Обозначим также $R \setminus \{e\} = \widehat{R}$. Тогда $R' = \widehat{R} \cup \{e'\}$.
%Возьмем случайное подмножество $\overline{R} \subseteq R'$, предположим $\forall \overline{R} \subseteq R: \overline{R} \in W \lor (\forall e \in Reach^w(\{\overline{R}\}) \exists e' \in R': e \sqsubseteq e'))$ и покажем, что определение~\ref{cpa_update_transfer_eq_power} удовлетворяет инварианту~\ref{cpa_update_transfer_eq}. 
%
%\begin{itemize}
%\item $e \notin \overline{R} \land e \notin Reach^w(\{\overline{R}\})$. Предположение $\overline{R}$ выполнено, так как
%\begin{align}
...
%\end{align}
%
%\item $\overline{R} \subseteq \widehat{R} \land e \in Reach^w(\{\overline{R}\})$. Для этого $\overline{R}$ предположение выполнено.
%\end{itemize}
%\fi
